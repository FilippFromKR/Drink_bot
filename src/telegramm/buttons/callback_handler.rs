...
async fn send_setting_message(bot: &AutoSend<Bot>, dialogue: &LocalDialogue, lang: &Lang) -> ReturnTy {
    let settings = CommandsHandler::get_settings(dialogue).await?;
    let keyboard = SettingsKeyboard::as_array()
        .iter()
        .map(|key| lang.buttons.settings.get(key.as_str()).expect("Unexpected code.").as_str())
        .collect::<Vec<&str>>();
...
async fn cocktail_for_you(bot: AutoSend<Bot>, dialogue: LocalDialogue) -> ReturnTy {
    ...
    if let Some(result) =
    DrinksService::search_by_first_letter(&random_char_of_eng_alphabet, user_settings.lang.clone()).await?
    {
...
fn ingredients_as_str_vec(raw_drink: &[LangDrink]) -> Vec<&str> {
    raw_drink
        ...
        .filter_map(|drink| match &drink.get_drink().ingredients.get(0) {
            None => None,
            Some(_) => {
                let random_ingredient = random_num_in_range(0, drink.get_drink().ingredients.len());
                Some(drink
                    .get_drink()
                    .ingredients
...
fn filter(filter: &str, vec: &mut Vec<LangDrink>) {
    vec.retain(|drink| !drink.get_drink().ingredients.iter().any(|(ingr, _)| ingr == filter)
    );
}
